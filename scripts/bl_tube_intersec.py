...
rospy.init_node("bl_tube_intersec")
...
sub_fixed_base = rospy.Subscriber(
    "bl_fixed_base_num", Int32, fixed_base_position_callback)
...
while not rospy.is_shutdown():
    if world_bl_rot_x is not None and world_bl_rot_y is not None and world_bl_tube_x is not None and world_bl_tube_y is not None and fixed_base_num is not None:
        w_result = getInterSectionPoint.calLine(
            center_x=world_bl_rot_x,
            center_y=world_bl_rot_y,
            p1_x=bl_tube_way_points[fixed_base_num][0],
            p1_y=bl_tube_way_points[fixed_base_num][1],
            p2_x=bl_tube_way_points[fixed_base_num + 1][0],
            p2_y=bl_tube_way_points[fixed_base_num + 1][1]
        )
        print(w_result)
