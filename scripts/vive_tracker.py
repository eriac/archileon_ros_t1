        ...
        for deviceName in v.devices:
            ...
            if "LHR" in v.devices[deviceName].get_serial():
                [qx, qy, qz, qw] = tf.transformations.quaternion_multiply(
                    [qx, qy, qz, qw], tf.transformations.quaternion_from_euler(math.pi, 0.0, -math.pi/2.0)
                    )
            ...
            if deviceName not in publisher:
                publisher[deviceName] = rospy.Publisher(publish_name_str, Float32MultiArray, queue_size=10)
