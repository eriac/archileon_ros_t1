...
def vive_tracker():
    ...
    p_cov[0, :] = [0.0000349162103240595,  -0.0000018202960310455,  -0.0000339898160507969,
                   -0.0000081126791170800,   0.0000001353045808767,   0.0000032202291901186]
    p_cov[1, :] = [-0.0000018202960310455,   0.0000011910722363973,   0.0000020423436706964,
                   0.0000010961526869235,  -0.0000000333091396801,  -0.0000001408541892558]
    p_cov[2, :] = [-0.0000339898160507969,   0.0000020423436706964,   0.0000341312090595451,
                   0.0000060715616751347,  -0.0000000237628610568,  -0.0000029217229365340]
    p_cov[3, :] = [-0.0000081126791170800,   0.0000010961526869235,   0.0000060715616751347,
                   0.0000165832615351042,  -0.0000004759697840205,  -0.0000024486872043021]
    p_cov[4, :] = [0.0000001353045808767,  -0.0000000333091396801,  -0.0000000237628610568,
                   -0.0000004759697840205,   0.0000003366392930324,  -0.0000000030521109214]
    p_cov[5, :] = [0.0000032202291901186,  -0.0000001408541892558,  -0.0000029217229365340,
    ...
    while not rospy.is_shutdown():
        ...
        for deviceName in v.devices:
            ...
            if 'Null' in v.devices[deviceName].get_serial():
                ...
                break
            publish_name_str = v.devices[deviceName].get_serial().replace(
                "-", "_")
            ...
            if deviceName not in publisher:
                publisher[deviceName] = rospy.Publisher(
                    publish_name_str, Float32MultiArray, queue_size=10)
            ...
            if "reference" not in deviceName:
                if deviceName + "_odom" not in publisher:
                    ...
                    publisher[deviceName + "_pose"] = rospy.Publisher(
                        publish_name_str + "_pose", PoseWithCovarianceStamped, queue_size=10)
                ...
                # set the position
                [inv_qx, inv_qy, inv_qz, inv_qw] = tf.transformations.quaternion_multiply(
                ...
                [vx, vy, vz, v_roll, v_pitch,
                    v_yaw] = v.devices[deviceName].get_velocities()
                ...
                if not (x == 0.0 and y == 0.0 and z == 0.0 and vx == 0.0 and vy == 0.0 and vz == 0.0 and v_roll == 0.0 and v_pitch == 0.0 and v_yaw == 0.0):
                    ...
                    try:
                        ...
                        odom.pose.pose = Pose(
                            Point(ring_trans[0], ring_trans[1], ring_trans[2]), Quaternion(qx, qy, qz, qw))
