...
void get_pose(float &pos_x, float &pos_y, float &rot_z)
{
  ...
  source_pose.header.frame_id = "tubeL_link1";
  source_pose.pose.orientation.w = 1.0;
  geometry_msgs::PoseStamped target_pose;

  tflistener.waitForTransform("world", "tubeL_link1", ros::Time(0), ros::Duration(1.0));
  tflistener.transformPose("world", ros::Time(0), source_pose, "tubeL_link1", target_pose);
...
{
  ros::init(argc, argv, "tf_bl_tube_listen");
  ros::NodeHandle n;

  ros::Publisher status_pub = n.advertise<std_msgs::Float32MultiArray>("bl_tube_status", 10, true);
